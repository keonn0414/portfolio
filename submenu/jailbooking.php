	...
	<div class="panel-heading" id="panel">
		<div class="media">
			...
			<div class="media-body">
				<h3>List of Inmate Jail booking(s)</h3>
				<span class="pull-right"><a onclick="PrintJailList()" class="btn btn-success">Print</a></span>
			</div>
		</div>
	</div>
	<div class="panel panel-body" id="">
			<table class="table table-hover table-filter">
				<tbody id="proposal_list">
					...
					$result = $conn->query("SELECT * FROM `tbl_jailbooking` order by Inmate_ID");
					...
					<tr data-status="pagado" class='alert alert-info'>
						...
						<div class="media">
							<div class="media" id='<?php echo $row['id']; ?>'>
								...
								<p class="title" style="font-weight: 900;">
									<?php
										echo strtoupper($row['Case_no']. " - ".$row['offense'])." <small>(" . $Name ." - ".$EID.")</small>";
									?>
								</p>
								<p class="summary"><?php echo $row['ArrrestingOfficer']; ?></p>
							</div>
						</div>
						</td>
					</tr><?php }
						...
						echo '<tr><td>No Jail Booking found.</tr>';
					}
					?>
				</tbody>
			</table>
